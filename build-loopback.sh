...
# Detach any loop devices still pointing at our old image
losetup -a | grep "pi5-nixos-8G.img" | awk -F: '{print $1}' \
  | xargs -r -n1 losetup -d || true

# Remove stale mount directories (if they exist)
...
echo "→ Bind‐mounting /dev, /proc, /sys, /run into the image…"
